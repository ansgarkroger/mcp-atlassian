...
| `JIRA_ATTACHMENT_DOWNLOAD_DIR` | Lets `jira_download_attachments` save files to disk when called with `target_dir`, confined to this directory (a relative `target_dir` resolves against it). Unset = attachment content is only returned inline. |
