...
| `comment_limit` | `integer` | No | Maximum number of comments to include (0 or null for no comments). The newest comments are returned, oldest first; 'comments_total' in the result is the size of the whole thread, so raise the limit when it exceeds the comments shown. |
